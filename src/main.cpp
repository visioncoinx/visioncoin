...
#define REWARD_CUT_BLOCK 5000000
...
uint256 hashGenesisBlock("0x00000dbb6d8e48bdce630aec7deb6734a74b4512a43890af6695dc0175656b7a");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // VisionCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "VisionCoin Signed Message:\n";
...
{
    // VisionCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Launch specs
static const int64 nTargetTimespan = 1 * 60; // VisionCoin: each block
static const int64 nTargetSpacing = 1 * 60; // VisionCoin: 60 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // VisionCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, VisionCoin - DarkGravity, written by Evan Duffield - evan@VisionCoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // VisionCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // VisionCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0000099ae60135984cc72acedc642fd243c25f555c47abd02ad8b0235e5ce1c9");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
      ...
      //vMerkleTree: 3f66108684400d84e2ffb528360d534fa19ecaf47c7c4c2ca1d1352ec4a351ad
        ...
        // Genesis block
        const char* pszTimestamp = "Vision";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xa6cc3024f2a291a68299e45271a249efc2e4445e80438ede0b1565b04f539f78"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa6, 0xd2, 0xc3, 0xf6 }; // VisionCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VisionCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VisionCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VisionCoinMiner(CWallet *pwallet)
{
    printf("VisionCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("VisionCoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running VisionCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("VisionCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VisionCoinMiner, pwallet));
}
